...
import com.sharetreats01.viber_chatbot.client.order.dto.OrderDataDto;
...
public class TreatDataUtils {
    ...
    /**
     *  tracking_data에서 productId 문자 부분을 추출하는 함수
     *  case 1 : ...PRODUCTS_{productID}:DETAIL:TREAT
     *  case 2 : ...PRODUCTS_{productID}:TREAT부분에서 productId 추출
     * @param treatParts - treat input 추출 부분들
     * @return productId(String)
     */
    public String getProductId(List<String> treatParts) {
        String detailPart = treatParts.stream()
            .filter(s -> s.startsWith(State.PRODUCTS.name())
                    || s.startsWith(State.DETAIL.name()))
            .findFirst()
            .orElse("");
